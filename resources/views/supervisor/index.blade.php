...
@section('titulo','Lista de Supervisores - Listagem | RH MAIS')
@section('conteudo')
                ...
                <div class="row">
                    <div class="col-md-12 col-sm-12 col-xs-12">
                        ...
                        <div id="lateral" class="suave">
                            ...
                            <h4 class="barlow">Cadastrar Supervisor<i class="material-icons click suave fechar">close</i></h4>
                            <form id="form-criar" method="POST">
                                {{-- <input type="hidden" name="_token" value="{{csrf_token()}}"> --}}
                                <div class="metade esquerda">
                                    <label>Nome</label>
                                    <input type="text" placeholder="Supervisor" name="nome" id="nome">
                                </div>
                            ...
                            <div id="editar" class="content suave">
                                <h4 class="barlow">Editar Supervisor<i class="material-icons click suave fechar">close</i></h4>
                                <form id="form-editar" method="post">
                                    <input type="hidden" name="e_id_supervisor" id="e_id_supervisor" value="">
                                    <div class="metade esquerda">
                                    <label>Nome</label>
                                    <input type="text" placeholder="Supervisor" name="e_nome" id="e_nome">
                                </div>
                                <div class="metade direita">
                                    <label>CPF</label>
...
<script>
    ...
    function carregar(){
        table = $('#tLista').DataTable({
            ajax: {
                url: 'supervisor-lista',
                dataSrc: ''
            },
            columns: [
                {data: 'id_supervisor', width: "60px", className: 'dt-body-center dt-head-center'},
                {data: 'nome_supervisor', width: "400px"},
         ...
         $(document).on("click", ".editar", function(){
            ...
            $("#editar #e_id_supervisor").val(data.id_supervisor);
            consultar(data.id_supervisor);
        });
        var delete_id;
        $(document).on("click", ".deletar", function(){
            delete_id = table.row($(this).parents("tr")).data();
            $("#alerta p").empty();
            $("#alerta p").text("Deseja realmente deletar esta supervisor?");
            $("#alerta .opcoes").removeClass("hide");
            $("#alerta").addClass("active");
        });
        $("#alerta .confirmar").click(function(){
            $(this).prop('disabled', true);
            deletar(delete_id.id_supervisor);
        });
    }carregar();
    ...
    $(".criar").click(function(){
        ...
        function criar(){
            ...
            request = $.ajax({
                ...
                processData: false,
                error: function(){
                    criaAlerta(0, "Falha ao criar supervisor!",2000);
                }
            });
            request.done(function(response){
                if(response == "1"){
                    ...
                    criaAlerta(0, "Supervisor criado!",5000);
                    recarregar();
                }
            });
        }
        ...
        function consultar(id){
            ...
            request.done(function(response){
                $('#form-editar input[name="e_id_supervisor"]').val(response.id_supervisor);
                $('#form-editar input[name="e_nome"]').val(response.nome_supervisor);
        ...
        function editar(){
            ...
            request = $.ajax({
                ...
                processData: false,
                error: function(){
                    criaAlerta(0,"Falha ao editar supervisor!",2000);
                }
            });
            request.done(function(response){
                if(response == "2"){
                    ...
                    criaAlerta(0, "Supervisor atualizado!",5000);
                    recarregar();
                }
            });
        }
        ...
        function deletar(id){
            request = $.ajax({
                ...
                type: 'delete',
                error: function(){
                    criaAlerta(0, "Falha ao deletar supervisor!",2000);
                }
            });
            request.done(function(response){
                if(response == "1"){
                    $("#alerta .opcoes").addClass("hide");
                    $("#alerta p").text("Supervisor deletado!");
